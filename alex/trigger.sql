...
CREATE OR REPLACE FUNCTION testCond(id int, support VARCHAR(50), temps VARCHAR(50), nb int) 
 RETURNS BOOLEAN AS $$
 	...
 	BEGIN 
 		IF support = 'day' THEN
 			IF temps = 'first' THEN
 				SELECT date INTO date1 FROM Representations WHERE idRepresentation = id;
 				return (date + interval nb 'day' >= (SELECT date FROM DateCourante) AND date >= (SELECT date FROM DateCourante) );
			ELSE
				SELECT date INTO date1 FROM Representations WHERE idRepresentation = id;
 				return (date - interval nb 'day' <= (SELECT date FROM DateCourante) AND date >= (SELECT date FROM DateCourante)); 
			END IF;
		ELSIF support = 'billet' THEN
			IF temps = 'first' THEN
				SELECT count(*) INTO nbPlace FROM Tickets WHERE idRepresentation=id;			
				return (nb-nbPlace)>0;	
 			ELSE
				SELECT nbPlaces INTO nbPlace FROM Representations WHERE idRepresentation=id;			
...
representation int) RETURNS void AS $$
    ...
    BEGIN
        ...
        SELECT reduction INTO reduction FROM Representations WHERE idRepresentation=representation;
        SELECT reducNombre INTO nb FROM Representations WHERE idRepresentation=representation;
        SELECT support INTO support FROM Representations WHERE idRepresentation=representation;
        SELECT temps INTO temps FROM Representations WHERE idRepresentation=representation;
        IF reduction IS NULL OR nb IS NULL OR support IS NULL or temps IS NULL THEN
        	prix := 0;
        ELSE
        	SELECT * INTO test FROM testCond(idReference, support, temps, nb);
        	SELECT prix INTO prix FROM Tarifs WHERE idRepresentation=representation AND nom='Normal';
